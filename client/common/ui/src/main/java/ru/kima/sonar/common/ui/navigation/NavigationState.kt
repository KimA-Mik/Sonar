package ru.kima.sonar.common.ui.navigation
...
//https://github.com/android/nav3-recipes/blob/main/app/src/main/java/com/example/nav3recipes/multiplestacks/NavigationState.kt
@Composable
private fun rememberRootNavBackStack(element: NavKey): NavBackStack<NavKey> {
    return rememberSerializable(
        element, serializer = NavBackStackSerializer(elementSerializer = NavKeySerializer())
    ) {
        NavBackStack(element)
    }
}
...
fun rememberNavigationState(
    ...
    val backStacks = topLevelRoutes.associateWith { key -> rememberRootNavBackStack(key) }
